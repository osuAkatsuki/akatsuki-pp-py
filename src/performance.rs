...
#[pymethods]
impl PyPerformance {
    fn is_relax(&self, py: Python<'_>) -> PyResult<bool> {
        if let Some(mods) = &self.mods {
            match PyGameMods::extract(Some(mods), GameMode::Osu, py) {
                Ok(PyGameMods::Lazer(mods)) => Ok(mods.iter().any(|m| m.acronym().as_str() == "RX")),
                Ok(PyGameMods::Intermode(mods)) => Ok(mods.iter().any(|m| m.acronym().as_str() == "RX")),
                Ok(PyGameMods::Legacy(mods)) => Ok(mods.contains(GameModsLegacy::Relax)),
                Err(_) => Ok(false),
            }
        } else {
            Ok(false)
    ...
    fn calculate(
        ...
        if self.is_relax(py)? && mode == GameMode::Osu {
            return self.calculate_2019(args, py);
        }
    ...
    fn calculate_2019(
        ...
        let map = args
            ...
            .map_err(|_| ArgsError::new_err("argument must be a Beatmap"))?;

        let mut perf = OsuPP::from_map(&map.inner);

        if let Some(mods) = &self.mods {
            let extracted_mods = PyGameMods::extract(Some(mods), GameMode::Osu, py)?;
            match extracted_mods {
                PyGameMods::Lazer(mods) => perf = perf.mods(mods),
                PyGameMods::Intermode(mods) => perf = perf.mods(mods),
                PyGameMods::Legacy(mods) => perf = perf.mods(mods),
            }
        }
        ...
        if let Some(accuracy) = self.accuracy {
            perf = perf.accuracy(accuracy as f32);
        }
        ...
        Ok(PyPerformanceAttributes::from(attrs))
    }
